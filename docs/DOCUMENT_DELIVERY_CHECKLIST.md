...
- 状态：`DECLARED`
- 交付文档：publication evidence、`PROJECT_STATUS`、`CAPABILITY_MATRIX`、`FINAL_REVIEW` 的发布事实更新。
- 直接读者：首次下载者、贡献者、项目负责人。
- 交付对象：真实仓库与 release 链接、固定 tag、六个制品、托管 CI 和公开下载后的安装／恢复结果。
- “发布完成”指上述入口真实可用且验证通过；当前版本仍为 `v0.1.0-alpha.2` 技术预览。
- 公开性：只登记公开 URL、摘要、合成示例和验证结论，不记录账户凭证、本机私人路径或用户数据。
- 首要质量判断：下载者能直接安装并恢复示例；证据供需要核对的人使用，不增加默认使用步骤。
